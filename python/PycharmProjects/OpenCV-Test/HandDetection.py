...
while (cap.isOpened()):
    ...
    for i in range(defects.shape[0]):
        ...
        if angle <= 90:
            ...
            # Far is space between fingertips
            cv2.circle(crop_img, far, 5, [0, 0, 255], -1)

            # End is the fingertips
            cv2.circle(crop_img, end, 5, [255, 0, 0], -1)

            # Start is the fingertips as well
            cv2.circle(crop_img, start, 5, [255, 0, 255], -1)


        # dist = cv2.pointPolygonTest(cnt,far,True)
        cv2.line(crop_img, start, end, [0, 255, 0], 4)
        # cv2.circle(crop_img,far,5,[0,0,255],-1)
    if count_defects == 1:
