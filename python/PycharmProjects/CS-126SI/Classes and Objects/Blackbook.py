class Blackbook:
    ...
    def __init__(self):
        print("Blackbook object was created")
    ...
    def encrypt(self, word, key):
        ...
        for letter in word:
            ...
            index = Blackbook.characters.index(letter.lower())
            ...
            index %= len(Blackbook.characters)
            ...
            encryptedWord += Blackbook.characters[index]
    ...
    def decrypt(self, word, key):
        ...
        for letter in word:
            ...
            index = Blackbook.characters.index(letter)
            index -= key
            ...
            decryptedWord += Blackbook.characters[index]
    ...
    def bruteForce(self, word, showAttempts=True):
        ...
        for attempt, currentWord in enumerate(words):
            ...
            for key in range(len(Blackbook.characters)):

                decryptedAttempt = self.decrypt(word, key)

                # If we want our program to show us every attempt (arg passed in to method parameters), then we'll do so.
                if showAttempts:
                    print("Attempt", attempt, "Comparing word", currentWord, "to:", decryptedAttempt)
                ...
                if decryptedAttempt.lower() == currentWord.lower():
                    print()
                    print("---------------------------- Match found! Attempt:", attempt, " Word is:", currentWord,
                          "Key is", key, "----------------------------")
                    return currentWord
...
encryptedText = blackbook.encrypt(word, key)
...
print(encryptedText, "decrypted via brute force is:", blackbook.bruteForce(encryptedText))